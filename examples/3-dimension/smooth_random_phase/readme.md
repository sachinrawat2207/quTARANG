...
where $\theta'(\vec{k},0)$ is the Fourier transform of $\theta(\vec{r},0)$, $\theta'(\vec{k},0)$ =  $\theta'^*(\bm{-k},0)$, $\alpha(\vec{k})$ is chosen randomly for each $\vec{k}$ in interval $[-\pi, \pi]$ and $\delta k = 2\pi/L$. 